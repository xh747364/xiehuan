 ...
 * -------------------------------更新日志-------------------------------
 * 
 * 2020.09.05 v1.01 1> 修复提示图标超出边框和不跟随物品移动问题
 * 
 * --------------------------------注意---------------------------------
 * 
 * 1.新物品提示(必须配置图片)
...
Window_ItemList.prototype.drawNewItemIcon = function (item, index) {
    if (item) {
        if ($gameSystem.isNewItem(item)) {
            ...
            if (rect.y > 0 && rect.y < this.innerHeight) {
                ...
                this.addInnerChild(newIconSprite);
                this._newItemSprites.push(newIconSprite);
            }
        };
    }
};
Window_ItemList.prototype.removeInnerChild = function (child) {
    this._innerChildren.remove(child);
    return this._clientArea.removeChild(child);
};
...
Game_System.prototype.isNewItem = function (item) {
    if (item) {
        ...
        if (currentType) { return !$gameSystem._lookItems[currentType][item.id] }
